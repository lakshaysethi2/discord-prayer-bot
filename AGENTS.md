...
| **Draw channel cmd** | `bot/daily_draw_commands.py` | `/setticketdrawchannel` validate/persist (issue #49). Dashboard Servers picker writes the same row. |
...
- **Daily draw channel (issue #49).** `/setticketdrawchannel` (admin slash + channel dropdown) and the dashboard Servers picker. DB `daily_draw_config.channel_id` wins over env after first seed. Not a mention-prefix command.
