...
## `/setticketdrawchannel` (issue #49)
Admin slash command with a text-channel dropdown. Dashboard Servers page writes the same `daily_draw_config.channel_id`. Not a mention-prefix command. Must not replace or wrap `on_interaction`.

## Persistent draw button
`DailyDrawTicketView` is `timeout=None` with static custom_id `daily_draw:ticket`, registered via `add_view` in `setup_hook`. Clicks go through `_respond_daily_draw_button` (view callback and `on_interaction` share it; `is_done()` + a lock prevent double-handle).
